...
export default async function Download() {
  try {
    ...
    const pathAnswer = await inquirer.prompt({
      ...
      rootPath: home,
      depthLimit: 2,
      excludePath: (p) => p.startsWith('node_modules') || p.includes(".") || p.includes("AppData") || p.toLowerCase().includes("programfiles"),
    ...
    console.log(`${chalk.green("Downloading")} ${chalk.cyan(`${options.filename}.${options.extension}`)}`);
...
async function DownloadVideo(options) {
  const outputFilePath = path.join(options.path, `${options.filename}.${options.extension}`);
  const tempOutputPath = path.join(options.path, `${options.filename}-temp.${options.extension}`);
  ...
  video.on('end', async () => {
    ...
    process.stdout.write(chalk.green(`Download complete. Converting to ${options.extension}\n\n`));
